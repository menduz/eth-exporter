import { RequestManager, HTTPProvider, SolidityEvent, Callback } from "eth-connect"
import { promisify } from "util"
import { cacheHit, fetchWithAttempts, readCache, sha256hash, writeCache } from "./fetcher"

function providerWithCache(provider: { sendAsync(data: any, callback: Callback): void }) {
  const sendAsyncPromise = promisify<any, any>(provider.sendAsync.bind(provider))

  return {
    sendAsync(data: any, callback: Callback) {
