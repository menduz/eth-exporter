...
import { Transfer, Account, Graph, normalizeAddress, filterTransfer, operationType } from "./graph.mjs"
...
function drawEdges(graph: Graph) {
  ...
  for (let [_, txlist] of graph.transactions) {
    for (let tx of txlist) {
      ...
      const type = operationType(graph, tx.hash)

      if (!graph.options.filter.length || graph.options.filter.includes(type)) {
        ...
        list.push(`<font color="${isEth ? "grey" : "black"}">${txLabel(tx)}&nbsp;${txDate(tx)} ${type}</font>`)
      }
