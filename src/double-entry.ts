...
import { InternalTx } from "./api"
import { txValue } from "./draw"
import { Account, filterTransfer, getAccountFromAddress, Graph, normalizeAddress, operationType, Options, Transfer } from "./graph"
...
export function doubleEntryFromGraph(graph: Graph) {
  ...
  function addLineItem(transfer: Transfer) {
    ...
    if (
      !graph.ignoredSymbols.has(transfer.contractAddress) &&
      !graph.ignoredSymbols.has(transfer.tokenSymbol || "????????")
    ) {
      const prev = contractToToken.get(transfer.contractAddress)
      const symbol = transfer.tokenSymbol || "????"
      if (prev && prev !== symbol) throw new Error("OOPS")
      contractToToken.set(transfer.contractAddress, symbol)
    }
    ...
    if (internalTxs) {
      for (let tx of internalTxs) {
        ...
        lineItem.changes.push({
          ...
          accountDebit,
          accountCredit,
          symbol: tx.tokenSymbol || tx.contractAddress,
          amount: txValue(tx),
          originalTx: tx,
        })
      }
    ...
    lineItem.fees = lineItem.fees.plus(new BigNumber(transfer.gasUsed).multipliedBy(transfer.gasPrice))
